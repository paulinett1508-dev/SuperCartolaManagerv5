...
function renderizarMataMata(data, timeId) {
    ...
    for (const edicao of data.edicoes) {
        if (edicao.confrontos && edicao.confrontos.length > 0) {
            const meuConfronto = edicao.confrontos.find(
                (c) =>
    ...
    let html = `
        <div style="padding: 20px;">
            <h2 style="margin: 0 0 20px 0; font-size: 22px; font-weight: 800; color: #fff;">⚔️ Mata-Mata</h2>
    `;

    if (minhaParticipacao && edicaoAtual) {
        // Participante está no Mata-Mata
        const sou_timeA =
            String(minhaParticipacao.timeA.timeId) === String(timeId);
        const eu = sou_timeA
            ? minhaParticipacao.timeA
            : minhaParticipacao.timeB;
        const adversario = sou_timeA
            ? minhaParticipacao.timeB
            : minhaParticipacao.timeA;
        ...
        html += `
            <div style="background: linear-gradient(135deg, rgba(255, 69, 0, 0.1) 0%, rgba(255, 69, 0, 0.05) 100%); border-radius: 12px; padding: 20px; margin-bottom: 20px; border: 2px solid rgba(255, 69, 0, 0.3);">
                <h3 style="margin: 0 0 16px 0; font-size: 18px; font-weight: 700; color: #fff;">🎯 Seu Confronto - ${edicaoAtual.nome}</h3>
                ...
                <div style="display: grid; grid-template-columns: 1fr auto 1fr; gap: 16px; align-items: center;">
                    ...
                    <div style="text-align: center; background: rgba(34, 197, 94, 0.1); padding: 16px; border-radius: 10px; border: 2px solid rgba(34, 197, 94, 0.3);">
                        ...
                        <div style="font-size: 16px; font-weight: 700; color: #fff; margin-bottom: 4px;">${eu.nomeTime || "Seu Time"}</div>
                        <div style="font-size: 24px; font-weight: 900; color: #22c55e;">${eu.pontos || 0}</div>
                    </div>

                    <!-- VS -->
                    <div style="font-size: 24px; font-weight: 900; color: #666;">VS</div>
                    ...
                    <div style="text-align: center; background: rgba(239, 68, 68, 0.1); padding: 16px; border-radius: 10px; border: 2px solid rgba(239, 68, 68, 0.3);">
                        ...
                        <div style="font-size: 16px; font-weight: 700; color: #fff; margin-bottom: 4px;">${adversario.nomeTime || "Adversário"}</div>
    ...
    html += `
        <h3 style="margin: 24px 0 16px 0; font-size: 18px; font-weight: 700; color: #fff;">📋 Histórico de Edições</h3>
        <div style="display: grid; gap: 12px;">
    `;

    data.edicoes.forEach((edicao) => {
        const totalConfrontos = edicao.confrontos
            ? edicao.confrontos.length
            : 0;
        html += `
            <div style="background: rgba(255,255,255,0.03); border-radius: 10px; padding: 16px; border: 1px solid rgba(255, 69, 0, 0.1);">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        <h4 style="margin: 0 0 4px 0; font-size: 16px; font-weight: 700; color: #fff;">${edicao.nome}</h4>
