...
router.get("/:ligaId/rodadas", async (req, res) => {
  ...
  try {
    ...
    if (rodada !== undefined) {
      ...
      console.log(
        `Buscando dados da rodada específica: ${rodadaNum} para liga ${ligaId}`,
      );
      ...
      if (inicioNum === fimNum) {
        rodadaNum = inicioNum;
        console.log(
          `Buscando dados da rodada específica: ${rodadaNum} (via inicio/fim) para liga ${ligaId}`,
        );
      } else {
        // Se inicio != fim, retornar range
        console.log(
          `Buscando range de rodadas ${inicioNum}-${fimNum} para liga ${ligaId}`,
        );
        ...
        console.log(
          `Encontrados ${dadosRodada.length} documentos para rodadas ${inicioNum}-${fimNum}`,
        );
    ...
    console.log(
      `Encontrados ${dadosRodada.length} documentos para a rodada ${rodadaNum}`,
    );
